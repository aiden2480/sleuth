...
async def chat(request):
    ...
    print(f"provided auth: {auth}\nCorrect auth: {cauth}")
    
    # Auth checks
    if not cauth:
        # The server has not created a token because the use never logged in
        return web.HTTPFound("/")
        return web.Response(text= "You are unauthorized. Please go back to the login page and sign in")
    if cauth != auth:
        # Wrong token
        return web.HTTPFound("/")
        return web.Response(text= "This authorization token is invalid. You will need to sign in again")
